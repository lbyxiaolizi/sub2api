...
// BindStickySession sets a session-to-account binding with the group's effective TTL.
...
// BindStickySessionAfterProfitAdmissionForGroup 同 BindStickySessionAfterProfitAdmission，
// 但绑定使用分组 TTL（Kiro 分组的粘性会话 TTL 可配置，默认 3600s）。
func (s *GatewayService) BindStickySessionAfterProfitAdmissionForGroup(ctx context.Context, groupID *int64, sessionHash string, accountID int64, group *Group) error {
	if sessionHash == "" || accountID <= 0 || s.cache == nil {
		return nil
	}
	if !gatewayProfitControlGateActive(ctx) {
		return s.BindStickySessionForGroup(ctx, groupID, sessionHash, accountID, group)
	}
	existingAccountID, err := s.cache.GetSessionAccountID(ctx, derefGroupID(groupID), sessionHash)
	if err != nil && !errors.Is(err, ErrStickySessionNotFound) {
		slog.Warn("profit_control_sticky_binding_read_failed", "group_id", derefGroupID(groupID), "account_id", accountID, "error", err)
