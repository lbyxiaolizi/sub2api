...
func (_q *ProxyQuery) QueryPool() *ProxyPoolQuery {
...
func (_q *ProxyQuery) WithPool(opts ...func(*ProxyPoolQuery)) *ProxyQuery {
	query := (&ProxyPoolClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *ProxyQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Proxy, error) {
	...
	if query := _q.withPool; query != nil {
		if err := _q.loadPool(ctx, query, nodes, nil,
			func(n *Proxy, e *ProxyPool) { n.Edges.Pool = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *ProxyQuery) loadPool(ctx context.Context, query *ProxyPoolQuery, nodes []*Proxy, init func(*Proxy), assign func(*Proxy, *ProxyPool)) error {
	ids := make([]int64, 0, len(nodes))
	nodeids := make(map[int64][]*Proxy)
	for i := range nodes {
...
func (_q *ProxyQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withPool != nil {
			_spec.Node.AddColumnOnce(proxy.FieldPoolID)
		}
	}
