...
func (_q *AccountQuery) QueryPool() *ProxyPoolQuery {
	query := (&ProxyPoolClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (_q *AccountQuery) WithPool(opts ...func(*ProxyPoolQuery)) *AccountQuery {
	query := (&ProxyPoolClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *AccountQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Account, error) {
	...
	if query := _q.withPool; query != nil {
		if err := _q.loadPool(ctx, query, nodes, nil,
			func(n *Account, e *ProxyPool) { n.Edges.Pool = e }); err != nil {
			return nil, err
		}
	}
...
func (_q *AccountQuery) loadPool(ctx context.Context, query *ProxyPoolQuery, nodes []*Account, init func(*Account), assign func(*Account, *ProxyPool)) error {
	ids := make([]int64, 0, len(nodes))
	nodeids := make(map[int64][]*Account)
	for i := range nodes {
	...
	query.Where(proxypool.IDIn(ids...))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	}
...
func (_q *AccountQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withPool != nil {
			_spec.Node.AddColumnOnce(account.FieldPoolID)
		}
