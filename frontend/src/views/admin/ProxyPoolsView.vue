<template>
  <AppLayout>
    ...
    >
      ...
      <template #footer>
        <div class="flex justify-end gap-2">
          <button class="btn btn-secondary" @click="showAssign = false">{{ t('common.cancel') }}</button>
...
import type { Proxy, ProxyPoolWithStats, ProxyPoolRebindLog } from '@/types'
...
const boundAccountSum = computed(() =>
  detailProxies.value.reduce((sum, p) => sum + (p.account_count ?? 0), 0)
)
...
async function openDetail(pool: ProxyPoolWithStats) {
  ...
  detailProxies.value = []
  rebindLogs.value = []
  await Promise.all([loadDetailProxies(pool.id), loadRebindLogs(pool.id)])
}
...
async function handleRebind() {
  ...
  try {
    const rebound = await adminAPI.proxyPools.rebind(detailPool.value.id)
    appStore.showSuccess(
      t('admin.proxyPools.rebindDone', { count: String(rebound) })
    )
    await Promise.all([
