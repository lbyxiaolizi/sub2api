<template>
  <AppLayout>
        ...
        >
          ...
          <template #cell-proxy="{ row }">
            <div class="flex flex-col gap-1">
              ...
              <div v-else-if="row.pool_name" class="flex items-center gap-1">
                <span class="inline-flex items-center px-1.5 py-0.5 rounded text-xs font-medium bg-indigo-100 text-indigo-800 dark:bg-indigo-900 dark:text-indigo-200">
                  <Icon name="link" size="xs" class="mr-1" />{{ row.pool_name }}
                </span>
              </div>
              <span v-else class="text-sm text-gray-400 dark:text-dark-500">-</span>
    ...
    <CreateAccountModal :show="showCreate" :proxies="proxies" :pools="proxyPools" :groups="groups" @close="showCreate = false" @created="reload" />
    <EditAccountModal :show="showEdit" :account="edAcc" :proxies="proxies" :pools="proxyPools" :groups="groups" @close="showEdit = false" @updated="handleAccountUpdated" />
...
import type { Account, AccountPlatform, AccountSchedulerGroupScore, AccountType, AccountUsageInfo, Proxy as AccountProxy, ProxyPoolWithStats, AdminGroup, WindowStats, ClaudeModel, UpstreamBillingProbeSnapshot } from '@/types'
...
onMounted(async () => {
  ...
  const proxyPoolsRequest = typeof adminAPI.proxyPools?.list === 'function'
    ? adminAPI.proxyPools.list()
    : Promise.resolve([] as ProxyPoolWithStats[])
  const [proxiesResult, groupsResult, proxyPoolsResult] = await Promise.allSettled([
  ...
  if (proxyPoolsResult.status === 'fulfilled') {
    proxyPools.value = proxyPoolsResult.value
  } else {
    console.error('Failed to load proxy pools:', proxyPoolsResult.reason)
  }
