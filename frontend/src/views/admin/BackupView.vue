<template>
    ...
    <ConfirmDialog :show="showRemoveDialog" :title="t('admin.backup.actions.delete')" :message="removeDialogMessage" :confirm-text="t('common.delete')" :cancel-text="t('common.cancel')" :danger="true" @confirm="confirmRemove" @cancel="showRemoveDialog = false" />
    <!-- 分卷下载链接 -->
    <teleport to="body">
      <transition name="modal">
        <div
...
const removeDialogMessage = computed(() => {
  const archived = !!backups.value.find(record => record.id === pendingRemoveId.value)?.monthly_archive
  return t(archived ? 'admin.backup.archive.deleteConfirm' : 'admin.backup.actions.deleteConfirm')
})
...
async function submitRestorePassword() {
  ...
  restorePassword.value = ''
  pendingRestoreId.value = ''
  await restoreSelectedBackup(id, password)
}
...
async function restoreSelectedBackup(id: string, password: string) {
...
async function confirmRemove() {
  ...
  try {
    const archived = !!backups.value.find(record => record.id === pendingRemoveId.value)?.monthly_archive
    await adminAPI.backup.deleteBackup(pendingRemoveId.value, archived)
